            ...
            function () {
                ...
                socket.on('history', function (history) {
                    history.forEach(function (h) {
                        var li = doc.createElement('li'), newMessage='';
                        for(var i=0;i<h.message.length;i+=30){
                            newMessage += h.message.substr(i,i+30)+'\n';
                        }
                        li.appendChild(doc.createTextNode(newMessage));
                ...
                socket.on('update_chat', function (message) {
                    var li = doc.createElement('li'), newMessage='';
                    for(var i=0;i<message.length;i+=30){
                        newMessage += message.substr(i,i+30)+'\n';
                    }
                    li.appendChild(doc.createTextNode(newMessage));
                    chat_content.appendChild(li);
                    chat_content.parentElement.scrollTop = chat_content.parentElement.scrollHeight;
    ...
    */
    ...
    chat_area.addEventListener('keypress', function (e) {
        if (e.ctrlKey && e.keyCode == 10) {
            ...
            for(var i=0;i<e.target.value.length;i+=30){
                newMessage += e.target.value.substr(i,i+30)+'\n';
            }
