...
Keyboard.prototype.onRead = function(bytesRead) {
  var event = parse(this, this.buf, bytesRead);
  if( event ) {
    event.dev = this.dev;
...
function parse(input, buffer, bytesRead) {
    ...
    switch(bytesRead){
        ...
        if( buffer.readUInt16LE(offset.ev_key) === EV_KEY ) {
            event = {
                timeS: buffer.readUInt32LE(0),
                timeMS: buffer.readUInt32LE(8),
                keyCode: buffer.readUInt16LE(offset.keyCode)
            };

            event.keyId = findKeyID(event.keyCode);
            event.type = EVENT_TYPES[ buffer.readUInt32LE(offset.type) ];
